package com.pnla.onroadplus.z_version2.MenuFragments.Commands.adapter;
...
public class CommandsV2Adapter extends RecyclerView.Adapter<CommandsV2Adapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, final int position) {
        ...
        holder.btnSendCommand.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (listener != null) {
                    if (!commands.get(position).isSelected()) {
                        listener.sendCommand(commands.get(position).getRoutine(),commands.get(position).getImei() ,position);
                    } else {
                        listener.commandSent("Ya se ha enviado el comando: " + commands.get(position).getRoutine() + " a ");
                    }
                }
            }
        });
    ...
    public interface OnClickCommandListener {
        void sendCommand(String commandName, String imei, int position);
